...
class TXTCDDatasetJSON(BaseDataset):
    ...
    def load_json(self, json_path):
        ...
        struct_json = {}
        for item in data:
            abs_path = item['image_path']
            values = list(item.keys())[1:10]
            
            # 1. 使用原始绝对路径作为键
            struct_json[abs_path] = values
            
            # 2. 使用文件名作为键(最通用的方式)
            filename = os.path.basename(abs_path)
            struct_json[filename] = values
            
            # 3. 如果有 data_root,使用相对路径作为键
            if hasattr(self, 'data_root') and self.data_root:
                try:
                    rel_path = os.path.relpath(abs_path, self.data_root)
                    struct_json[rel_path] = values
                    
                    # 4. time1/time2 -> A/B 的兼容性
                    if 'time1' in rel_path:
                        rel_path_AB = rel_path.replace('time1', 'A')
                        struct_json[rel_path_AB] = values
                    elif 'time2' in rel_path:
                        rel_path_AB = rel_path.replace('time2', 'B')
                        struct_json[rel_path_AB] = values
    ...
    def load_data_list(self) -> List[dict]:
        ...
        if osp.isfile(self.ann_file):
            ...
            for line in lines:
                ...
                data_info['seg_fields'] = []
                # --- 修改开始 ---
                json_path = os.path.dirname(lines[0].strip().split('  ')[0]) + '_clipcls_56_vit16.json'
                if os.path.exists(json_path):
                    # 尝试多种键格式查找 JSON 数据,优先使用文件名(最通用)
                    # 1. 首先尝试使用文件名
                    keyA = os.path.basename(pimgA)
                    keyB = os.path.basename(pimgB)
                    
                    # 2. 如果文件名找不到,尝试绝对路径
                    if keyA not in jsondataA:
                        keyA = pimgA
                    
                    if keyB not in jsondataB:
                        keyB = pimgB
                    
                    # 3. 如果绝对路径找不到,尝试相对路径
                    if keyA not in jsondataA:
                        keyA = os.path.relpath(pimgA, self.data_root)
                    
                    if keyB not in jsondataB:
                        keyB = os.path.relpath(pimgB, self.data_root)
                    
                    # 4. 兼容性处理: time1/time2 -> A/B
                    if keyA not in jsondataA and 'time1' in str(keyA):
                        keyA = str(keyA).replace('time1', 'A')
                    
                    if keyB not in jsondataB and 'time2' in str(keyB):
                        keyB = str(keyB).replace('time2', 'B')

                    # 5. 赋值 (如果还报错,说明 JSON 文件内容和图片完全对应不上)
                    try:
                        data_info['jsonA'] = jsondataA[keyA]
                        data_info['jsonB'] = jsondataB[keyB]
                    except KeyError as e:
                        print(f"\n[Debug Error] Key not found in JSON.")
                        print(f"Looking for: {e}")
                        print(f"Original Abs Path A: {pimgA}")
                        print(f"Original Abs Path B: {pimgB}")
                        print(f"Tried Key A: {keyA}")
                        print(f"Tried Key B: {keyB}")
                        print(f"JSON A Keys Sample (first 5): {list(jsondataA.keys())[:5]}")
                        print(f"JSON B Keys Sample (first 5): {list(jsondataB.keys())[:5]}")
                        print(f"Total keys in JSON A: {len(jsondataA)}")
                        raise e # 重新抛出异常停止程序Sample (first 5): {list(jsondataB.keys())[:5]}")
                        raise e # 重新抛出异常停止程序
                else:
                    data_info['jsonA'] = []
                    data_info['jsonB'] = []
